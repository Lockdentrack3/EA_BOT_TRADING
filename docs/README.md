...
├── mql5/
│   ├── Experts/
│   │   └── EA_BotTrading.mq5          ← Main EA (compile & attach in MT5)
│   ├── Include/
│   │   ├── SignalEngine.mqh            ← Multi-factor signal engine
│   │   ├── RiskManager.mqh            ← Position sizing & risk controls
│   │   ├── TradeManager.mqh           ← BE, trailing stop, partial close
│   │   └── MarketRegime.mqh           ← Regime detection + news filter + logger
│   └── Scripts/
│
├── python/
│   ├── core/
...
│   │   ├── signal_engine.py           ← AI signal engine (Python)
│   │   ├── mt5_bridge.py              ← MT5 ↔ API bridge (run on Windows)
...
│
├── risk_management/
│   └── risk_manager.py                ← Institutional risk engine
│
├── analytics/
│   └── performance.py                 ← Sharpe, Sortino, drawdown, reports
│
├── backtest/
│   └── engine.py                      ← Backtest + Walk-Forward + Monte Carlo
│
├── telegram/
│   └── bot.py                         ← Full Telegram command panel
│
